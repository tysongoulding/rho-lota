...
export function CustomiseModal() {
  const { customiseModalOpen, setCustomiseModalOpen } = useUiStore();

  if (!customiseModalOpen) return null;

  return (
    <div
      onClick={() => setCustomiseModalOpen(false)}
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#30363d] bg-[#0d1117] flex-shrink-0">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-white text-sm">Appearance & Theme Customisation</span>
          </div>
          <button
            onClick={() => setCustomiseModalOpen(false)}
